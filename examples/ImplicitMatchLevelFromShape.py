...
def get_circle_data(implicit=True):
    ...
    if implicit:
        ...
        for circle_point_set in circle_data.transpose((1, 0, 2)):
            implicit_data.extend(circle_point_set)
            implicit_data.append(np.full(circle_data.shape[2], np.nan))
...
class ParentFitness(VectorBasedFunction):
    ...
    def evaluate_fitness_vector(self, individual):
        ...
        parent_string = parent_string.replace("X_3", self.child_agraph_str)
        full_agraph = AGraph(equation=parent_string)
...
def main():
    # TODO do we even need two steps for this or just use combined fitness function?
    # TODO actually just learn both implicit forms at once
    use_implicit = True
    ...
    # eq = AGraph(equation="X_3 - (X_2 * X_2 + X_2)**2 + 1")
    eq = AGraph(equation="X_3 / (X_2 * X_2 + X_2)**2")
    print(fitness(eq))
    ...
    component_generator = ComponentGenerator(4)
